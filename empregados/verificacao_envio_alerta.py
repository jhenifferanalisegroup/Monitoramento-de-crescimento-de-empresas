...
#------------------ Verificação de crescimento desde 02/2026 ------------------
    for _, row in df.iterrows():
        ...
        '''if crescimento_primeiro >= PERCENTUAL_ALERTA:
            print("🚨 ALERTA DE CRESCIMENTO DESDE 02/2026!\n")
            alertas_crescimento_primeiro.append(
                {
                    "tipo": "ALERTA DE CRESCIMENTO DESDE 02/2026",
        ...
        if crescimento_anterior >= PERCENTUAL_ALERTA:
            print("🚨 ALERTA DE CRESCIMENTO EM COMPARAÇÃO AO MÊS ANTERIOR!\n")
            alertas_crescimento_anterior.append(
                {
                    "tipo": "ALERTA DE CRESCIMENTO EM COMPARAÇÃO AO MÊS ANTERIOR!",
                    "codigo": codigo,
                    "cnpj": cnpj,
                    "empresa": empresa,
                    "total_primeiro": total_primeiro,
...
if __name__ == "__main__":
    executar_teste()
    conteudo_email = montar_email(informacoes_empresas)
    #= montar_email(alertas_crescimento_primeiro)
    #conteudo_email_anterior = montar_email(alertas_crescimento_anterior)

    if conteudo_email:
        enviar_email_alerta(conteudo_email)
        
    #if conteudo_email_anterior:
    #    enviar_email_alerta(conteudo_email_anterior)

    # Após o envio dos e-mails, salva um snapshot para cada empresa em alerta
